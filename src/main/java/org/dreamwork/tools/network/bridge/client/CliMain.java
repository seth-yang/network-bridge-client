...
import org.dreamwork.network.bridge.data.SystemConfig;
...
*/
    ...
    private void initDatabase (/*SQLite sqlite*/) throws IOException {
        if (root == null) {
            logger.error ("root configuration is not found !!");
            System.exit (-1000);
            return;
        }

        boolean sshdDisabled = root.getBoolean ("network.bridge.client.sshd.disabled", false);
        if (!sshdDisabled) {
